...
function body(heading: string, message: string, cta?: { label: string; href: string }) {
  const ctaBlock = cta
    ? `<p style="margin:24px 0 0"><a href="${cta.href}" style="background:#1a1510;color:#faf8f4;padding:10px 20px;font-family:monospace;font-size:11px;letter-spacing:0.1em;text-decoration:none;text-transform:uppercase">${cta.label}</a></p>`
    : "";
...
export async function sendTripApproved(to: string) {
  ...
  await resend.emails.send({
    ...
    html: body(
      ...
      { label: "View the feed →", href: `${SITE_URL}/feed` }
    ),
  });
}

export async function sendTripRejected(to: string) {
  const resend = getResend();
  if (!resend) return;
  await resend.emails.send({
    from: FROM,
...
export async function sendPostApproved(to: string) {
  ...
  await resend.emails.send({
    ...
    html: body(
      ...
      { label: "Go to community →", href: `${SITE_URL}/community` }
    ),
  });
}

export async function sendPostRejected(to: string) {
  const resend = getResend();
  if (!resend) return;
  await resend.emails.send({
    from: FROM,
...
export async function sendBewareApproved(to: string) {
  ...
  await resend.emails.send({
    ...
    html: body(
      ...
      { label: "View Beware Board →", href: `${SITE_URL}/community` }
    ),
  });
}

export async function sendBewareRejected(to: string) {
  const resend = getResend();
  if (!resend) return;
  await resend.emails.send({
    from: FROM,
